...
class Tick(Base):
    ...
    Indexed on (symbol, timeframe, time) for time-series queries.
    """
    __tablename__ = "ticks"
    __table_args__ = (
        sa.UniqueConstraint("symbol", "timeframe", "time", name="uq_tick"),
    )
    ...
    def __repr__(self) -> str:
        return f"<Tick({self.symbol} {self.timeframe} {self.time})>"
